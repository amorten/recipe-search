...
if __name__ == "__main__":
    ...
    cursor.execute("""select ing_nums.ingred_id,NaiveIngreds.ingred_text,ing_nums.cnt,ing_nums.cnt*ing_nums.cnt/1000000*4/3600/24 as runtime from (select ingred_id, count(ingred_id) as cnt from recipes_naiveingreds group by ingred_id order by count(ingred_id) desc) as ing_nums INNER JOIN SmartIngreds1 as sm1 on sm1.ingred_id=ing_nums.ingred_id INNER JOIN NaiveIngreds on ing_nums.ingred_id=NaiveIngreds.ingred_id order by cnt desc;""")
    ...
    print(runtimes[0:10])
    print("Total estimated runtime (CPU days) for single ingred clusters: {}".format(sum(runtimes)))
